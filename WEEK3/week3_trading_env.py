...
def run_sanity_check():
    """
    Confirm the environment satisfies the Gymnasium API contract.

    check_env() will warn that the observation space bounds are +/-inf.
    That is expected here (price returns are unbounded in principle) and
    is not an error -- the Week 4 assignment notes explicitly call this
    out as a non-critical warning, not something to "fix".
    """
    from gymnasium.utils.env_checker import check_env

    print("=" * 55)
    print("check_env sanity check")
    print("=" * 55)
